...
const PagePost = () => {
  const missingSpecs = useAppSelector((state) =>
    ItemSpecsSLice.selectors.getMissingSpecsName(state),
  );
  ...
  return (
    <>
      <Layout style={{ minHeight: "100vh", padding: 32, background: "none" }}>
        <Header
          style={{ margin: 0, padding: 0, background: "none", height: "auto" }}
        >
          <Flex vertical align="start" style={{ background: "none" }}>
            <Button type="link" onClick={() => navigate("/ads")}>
              ← Назад
            </Button>
            <Flex style={{ width: "100%" }} justify="space-between">
              {isLoading ? (
                ...
                >
                  <Skeleton.Input
                    active
                    style={{ width: 300, height: 40, display: "block" }}
                  />
                  <Skeleton.Button
                    active
                    style={{ width: 150, height: 32, display: "block" }}
                  />
                </div>
              ) : (
              ...
              {isLoading ? (
                ...
                >
                  ...
                  >
                    <Skeleton.Input
                      active
                      size="small"
                      style={{ width: 180, height: 16, display: "block" }}
                    />
                    <Skeleton.Input
                      active
                      size="small"
                ...
                <Flex vertical gap={12} align="end">
                  ...
                  <Flex vertical align="end">
                    <Typography.Text
                      style={{ color: "#848388", display: "block" }}
                    >
                      Опубликовано: {createDate(data?.createdAt!)}
                    </Typography.Text>
                    <Typography.Text
                      style={{ color: "#848388", display: "block" }}
                    >
                      Отредактировано: {createDate(data?.updatedAt!)}
                    </Typography.Text>
                  </Flex>
                </Flex>
              )}
            ...
            <Flex vertical gap={32}>
              ...
              <Flex vertical gap={16}>
                ...
                {data?.description ? (
                  ...
                  <Typography.Text style={{ color: "#848388" }}>
                    Отсутствует
                  </Typography.Text>
                )}
              </Flex>
