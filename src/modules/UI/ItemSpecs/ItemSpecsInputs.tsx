      ...
      {specsList.map((spec, index) => (
        <Flex vertical key={spec.key + index} gap={8}>
          ...
          { Object.keys(selectorsOptions).includes(spec.key) ? (
                ...
                <Input status={spec.value ? "" : "warning"} placeholder={spec.label} value={spec.value} style={{width: 456}} allowClear={true} onChange={(newValue) => onSelect(spec.key, newValue.target.value)}/>
            ) 
          }
        </Flex>
      ))}
